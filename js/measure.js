...
// ── TLAČÍTKA UI ──────────────────────────────────────────────────
function _msrSyncButtons() {
  const btnClose  = document.getElementById('msr-btn-close');
  const btnClear  = document.getElementById('msr-btn-clear');
  if (btnClose) btnClose.style.display = msrClosed ? 'none' : '';
...
function msrUpdate() {
  ...
  if (msrClosed && mPts.length >= 3) {
    // Po uzavření: 'Délka' → 'Obvod', hint skrytý
    document.getElementById('msr-lbl-val').textContent = 'Obvod';
    ...
    document.getElementById('msr-lbl-val').textContent = 'Délka';
    document.getElementById('msr-area').textContent = msrFormatArea(msrCalcArea(mPts)) + ' est.';
    const hintEl = document.getElementById('msr-hint');
    if (hintEl) { hintEl.style.display = ''; hintEl.textContent = 'Enter → uzavřít plochu'; }
  } else {
    document.getElementById('msr-lbl-val').textContent = 'Délka';
    document.getElementById('msr-area').textContent = '—';
    const hintEl = document.getElementById('msr-hint');
    if (hintEl) { hintEl.style.display = ''; hintEl.textContent = 'klikej body na mapě'; }
  }
}
...
function toggleMsr() {
  ...
  if (msrOn) {
    ...
    _msrSyncButtons();
    // Vypni interaktivitu IS DMVS vrstev — klik projde na mapu
    if (typeof qgisLayers !== 'undefined') {
      qgisLayers.forEach(l => { if (l.leaflet) l.leaflet.eachLayer(layer => {
        if (layer.options) layer.options.__msrSavedInteractive = layer.options.interactive;
...
function closeMsrPolygon() {
  ...
  // Obnov interaktivitu IS DMVS vrstev
  if (typeof qgisLayers !== 'undefined') {
    qgisLayers.forEach(l => { if (l.leaflet) l.leaflet.eachLayer(layer => {
      const was = layer.options?.__msrSavedInteractive;
      if (layer.setStyle) layer.setStyle({ interactive: was !== false });
    }); });
  }
  msrDraw();
  msrUpdate();
...
function clearMsr() {
  ...
  document.getElementById('msr-area').textContent = '—';
  const _h = document.getElementById('msr-hint');
  if (_h) { _h.style.display = ''; _h.textContent = 'klikej body · Enter → uzavřít'; }
  const _lv = document.getElementById('msr-lbl-val');
  if (_lv) _lv.textContent = 'Délka';
  _msrSyncButtons();
